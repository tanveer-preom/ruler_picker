...
final class RulerView extends View {
    ...
    private Paint mFutureIndicatorPaint;
     ...
     *
     * @see #setValueRange(long, long)
     * @see #getMinValue()
     */
    private long mMinValue = 0 /* Default value */;
     ...
     * @see #getMaxValue()
     */
    private long mMaxValue = 100 /* Default maximum value */;
    ...
    @ColorInt
    private int mFutureIndicatorColor = Color.GREEN;
    ...
    private void refreshPaint() {
        ...
        mFutureIndicatorPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mFutureIndicatorPaint.setColor(mFutureIndicatorColor);
        mFutureIndicatorPaint.setStrokeWidth(mIndicatorWidthPx);
        mFutureIndicatorPaint.setStyle(Paint.Style.STROKE);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int mins =  (int)((mMaxValue - mMinValue) / (1000 *60));
        for (int value = 1; value <mins; value++) {
            SimpleDateFormat format = new SimpleDateFormat("mm");
            int min = Integer.parseInt(format.format(new Date(mMinValue + value * 60 * 1000L)));
            if (min % 15 == 0) {
                drawLongIndicator(canvas, value);
                drawValueText(canvas, value);
            } else if(min%5 == 0) {
                drawMediumIndicator(canvas, value);
            } else{
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        long viewWidth = (mins - 1) * mIndicatorInterval;

        updateIndicatorHeight(mLongIndicatorHeightRatio, mShortIndicatorHeightRatio);
        mLongIndicatorHeight = (int) (mViewHeight/2);
        mShortIndicatorHeight = (int) (mLongIndicatorHeight/2);
        mIndicatorInterval = RulerViewUtils.dpTopx(getContext(),10);
        mTextSize = RulerViewUtils.sp2px(getContext(),13);
        mTextPaint.setTextSize(mTextSize);
    ...
    private void drawSmallIndicator(@NonNull final Canvas canvas,
                                    final int value) {
        Paint paint = mIndicatorPaint;
        if((mMinValue + value * 60 *1000L) >System.currentTimeMillis()){
            paint = mFutureIndicatorPaint;
        }
    ...
    private void drawMediumIndicator(@NonNull final Canvas canvas,
                                   final int value) {
        Paint paint = mIndicatorPaint;
        if((mMinValue + value * 60 *1000L) >System.currentTimeMillis()){
            paint = mFutureIndicatorPaint;
        ...
        canvas.drawLine(mIndicatorInterval * value,
                mViewHeight,
                mIndicatorInterval * value,
                mViewHeight - (mLongIndicatorHeight+mShortIndicatorHeight) / 2F,
                paint);
    }
    ...
    private void drawValueText(@NonNull final Canvas canvas,
                               final int value) {
        if((mMinValue + value * 60 *1000L) < System.currentTimeMillis()){
            canvas.drawText(new SimpleDateFormat("hh:mm").format(new Date(value *60 *1000L + mMinValue)),
                    mIndicatorInterval * value,
                    getHeight() - (mLongIndicatorHeight + RulerViewUtils.dpTopx(getContext(),5)),
                    mTextPaint);
        }
     ...
     */
    void setValueRange(final long minValue, final long maxValue) {
        mMinValue = minValue;
        mMaxValue = maxValue;
        invalidate();
    }
    ...
    public int getFutureMinutes() {
        ...
        for(int i = mins; i >= 0; i--) {
            if((mMinValue + i * 60 *1000L) > System.currentTimeMillis()) {
                futureMinutes++;
            } else {
                break;
            }
